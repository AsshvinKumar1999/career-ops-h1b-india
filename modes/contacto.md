...
1. **Identify targets** via WebSearch:
   ...
   - Assigned recruiter
   - 2-3 peers from the team (people with similar role)
   - Interviewer (if candidate already has interview scheduled)
...
4. **Generate message** with 3-sentence framework adapted to contact type:
   ...
   ### Hiring Manager
   - **Sentence 1 (Hook)**: Specific challenge their team faces (from JD, company blog, or news)
   - **Sentence 2 (Proof)**: Candidate's biggest quantifiable achievement that proves they've solved similar problems
   ...
   - **Sentence 2 (Context)**: Light connection to the candidate's experience with that topic
