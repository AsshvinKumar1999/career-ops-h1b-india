...
- `title_filter`: Positive/negative keywords and seniority_boost for title filtering
...
Levels are additive -- execute all, mix results and deduplicate.
...
5. **Level 2 -- ATS APIs / feeds** (parallel):
   ...
   d. For **BambooHR**, the list only brings basic metadata. For each relevant item, read `id`, GET `https://{company}.bamboohr.com/careers/{id}/detail`, and extract full JD from `result.jobOpening`. Use `jobOpeningShareUrl` as public URL if present; if not, use the detail URL.
...
6. **Level 3 -- WebSearch queries** (parallel if possible):
   ...
   b. From each result extract: `{title, url, company}`
      ...
      - **url**: URL from result
      - **company**: after the " @ " in title, or extract from domain/path
...
7.5. **Verify liveness of WebSearch results (Level 3)** -- BEFORE adding to pipeline:
   ...
   **Don't interrupt the entire scan if one URL fails.** If `browser_navigate` gives error (timeout, 403, etc.), mark as `skipped_expired` and continue with the next one.
...
Each company in `tracked_companies` should have `careers_url` -- the direct URL to their jobs page. This avoids searching for it each time.
...
- Add companies to `tracked_companies` when wanting to closely follow them
